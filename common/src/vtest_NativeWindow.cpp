...
NativeWindow::NativeWindow(OMX_BOOL bRotate,
	...
	VTEST_MSG_HIGH("m_nColorFormat = %d", m_nColorFormat);
...
PortBufferCapability NativeWindow::GetBufferRequirements(OMX_U32 ePortIndex) {
	...
	VTEST_MSG_HIGH("GetBufferRequirements = %d", ePortIndex);
...
OMX_ERRORTYPE NativeWindow::Configure(CodecConfigType *pConfig,
	...
	VTEST_MSG_MEDIUM("Configure  m_nMinBufferCount=%d",m_nMinBufferCount);
    return result;
}
...
OMX_ERRORTYPE NativeWindow::SetBuffer(
    ...
    if (pSource == m_pSource) {
		...
		VTEST_MSG_MEDIUM("SetBuffer  m_pSink queue get =(%p %p) ",pFreeBuffer->pHeaderIn,pBuffer->pHeaderOut);
...
OMX_ERRORTYPE NativeWindow::AllocateBuffers(BufferInfo **pBuffers,
	...
	VTEST_MSG_MEDIUM("AllocateBuffers  transform=%d ",transform);
	...
	if(GetGrallocFormat(m_nColorFormat) == HAL_PIXEL_FORMAT_YV12){
		VTEST_MSG_MEDIUM("AllocateBuffers  m_nColorFormat == HAL_PIXEL_FORMAT_YV12");
	}
	else{
		VTEST_MSG_MEDIUM("AllocateBuffers  m_nColorFormat == HAL_PIXEL_FORMAT_RGBA_8888");
	}
    ...
    if (nBufferUsage & GRALLOC_USAGE_PROTECTED) {
        ...
        // SurfaceFlinger.
        VTEST_MSG_MEDIUM("AllocateBuffers  GRALLOC_USAGE_PROTECTED");
	...
	//VTEST_MSG_MEDIUM("AllocateBuffers  nBufferCount=%d",nBufferCount);
...
OMX_ERRORTYPE NativeWindow::FreeAllocatedBuffers(BufferInfo **pBuffers,
	...
	VTEST_MSG_MEDIUM("FreeAllocatedBuffers  nBufferCount=%d",nBufferCount);
...
OMX_ERRORTYPE NativeWindow::ThreadRun(OMX_PTR pThreadData) {
     ...
     * hold stop until all buffers are cleared in the queue */
     VTEST_MSG_MEDIUM("ThreadRun  start");
...
OMX_ERRORTYPE NativeWindow::DequeueBuffer(OMX_BUFFERHEADERTYPE *& bufHdr) {
	...
	VTEST_MSG_MEDIUM("DequeueBuffer  sucess");
...
OMX_ERRORTYPE NativeWindow::EnqueueBuffer(OMX_BUFFERHEADERTYPE *pHeader) {
    ...
    VTEST_MSG_MEDIUM("EnqueueBuffer Displaying buffer [%p]", buf);
